...
function WelcomePage(props) {
  ...
  function findFirstEmptyRow(column) {
    ...
    for (let i = 0; i < rows; i++) {
      if (board[getInde(i, column)] !== empty) {
        return i - 1;
      }
    }
    return rows - 1;
  ...
  async function handleDrop(column) {
    ...
    newBoard[getInde(row, column)] = currentPlayer;
    setBoard(newBoard);
    // Check for win
  ...
  async function animateDrop(row, column, color, currentRow) {
    ...
    return new Promise((resolve) => {
      ...
      if (currentRow > 0) {
        let c = getDomBoardCell(getInde(currentRow - 1, column));
        c.style.backgroundColor = boardSetting.colors.empty;
      }
      let c = getDomBoardCell(getInde(currentRow, column));
  ...
  useEffect(() => {
    ...
    function flashWinningCells(on) {
      ...
      for (let o of win.winningCells) {
        let c = getDomBoardCell(getInde(o.row, o.column));
  ...
  useEffect(() => {
    ...
    function createWinState(start, winType) {
      ...
      while (true) {
        let current = board[getInde(pos.row, pos.column)];
    ...
    function isHorizontalWin() {
      ...
      for (let row = 0; row < rows; row++) {
        for (let column = 0; column <= columns - 4; column++) {
          let start = getInde(row, column);
          if (board[start] === empty) continue;
          let counter = 1;
          for (let k = column + 1; k < column + 4; k++) {
            if (board[getInde(row, k)] === board[start]) {
              counter++;
              if (counter === 4)
      ...
      for (let row = 0; row <= rows - 4; row++) {
        for (let column = 0; column < columns; column++) {
          ...
          for (let k = row + 1; k < row + 4; k++) {
            if (board[getInde(k, column)] === board[start]) {
              counter++;
              if (counter === 4)
    ...
    function isBackwardsDiagonalWin() {
      ...
      for (let row = 0; row <= rows - 4; row++) {
        for (let column = 0; column <= columns - 4; column++) {
          ...
          let counter = 1;
          for (let i = 1; i < 4; i++) {
            if (board[getInde(row + i, column + i)] === board[start]) {
              counter++;
              if (counter === 4)
